...
object GitHubAppAuth {
   ...
   */
  def fromConfigMap(configMap: Map[String, String], prefix: String): GitHubAppAuth = new GitHubAppAuth(
    new GitHubAppJWTs(
      appClientId = configMap(s"${prefix}_GITHUB_APP_CLIENT_ID"),
      GitHubAppJWTs.parsePrivateKeyFrom(configMap(s"${prefix}_GITHUB_APP_PRIVATE_KEY")).get
    ),
  )
}
