...
import com.madgag.scalagithub.{AccountCredentials, GitHub, GitHubCredentials}
...
class ToastRepoCreation(
  ...
  def createTestRepo(fileName: String)(implicit ec: ExecutionContext): IO[Repo] = {
    ...
    for {
      ...
      _ <- pushLocalRepoToGitHub(localGitRepo, testGithubRepo) >>
        Temporal[IO].sleep(0.seconds) >> // pause so we don't immediately cache results before the GitHub API catches up
        validateGitHubAPISuccessfullyRepresentsLocalRepo(testGithubRepo, localGitRepo)
  ...
  private def validateGitHubAPISuccessfullyRepresentsLocalRepo(
    testGithubRepo: Repo, localGitRepo: Repository
  )(using ExecutionContext): IO[Unit] = {
    val branchRefs: Seq[Ref] = localGitRepo.getRefDatabase.branchRefs
    
    // Option[(Ref, RefProblem)] give first ref that failed (after retries)
    IO.traverse(branchRefs) { branchRef =>
      fetchRefFromGitHubApiWithRetries(testGithubRepo, branchRef)
        .map(_.result.objectId ).attempt.map { a =>
          branchRef ->
            a.fold[Option[RefProblem]](t => Some(ErrorGettingRef(t)), b => Option.when(b!= branchRef.getObjectId)(WrongValue(b)))
        }
    }
    ...
    for {
      ...
      require(localCloneOfGitHubRepo.findRef(testGithubRepo.default_branch).getObjectId == localGitRepo.resolve("HEAD"))
    }
  }

  private def fetchRefFromGitHubApiWithRetries(testGithubRepo: Repo, branchRef: Ref)(using ExecutionContext, GitHub) = 
